...
//int beatCount (struct track, struct beat)
...
int add_note_to_beat(Beat beat, int octave, int key) {
	...
	//This variable checks for a valid note
	int returnVal = VALID_NOTE; 
...
void cut_range_to_end(Track track, int range_length) {
    ...
    struct beat *initialPrev = initialCurr;
...
int reverse_range(Track track, int range_length) {
	...
	if (range_length >= list_remain) {
		...
		range_length = list_remain;
	}
	//Beware! if the linked list has zero elements
	else if (range_length == 0 || range_length < 0) {
		return 0;
	}
	while ( i <= range_length ) {
...
//Convert the first char of the array to a number for the octave
int octave_num_check  (char musicArray[]) {
	int octave = 0;
	if (musicArray[0] == '0'){
		octave = 0;
...
//Convert the 2nd char of the array to a number for the key
int key_num_check (char musicArray []) {
	int key = 0;	
	if (musicArray[1] == 'A'){
		key = 0;
...
int music_to_beat(Beat beat, int octave, int key) {
	...
	while(curr != NULL) 
	{
		...
		{
		//If currKey is at the end of the list
		//then insert 
			...
			//the same octaves
			int listCheck = 0;
				while (currKey->key < key)
				{
					//Check if curr octave may be greater than
					//the next octave
					if (currKey->octave > octave ) 
					{	
						prev->next = newNote;
						newNote->next = currKey;
						return returnVal;
					...
					// currKey gets to the end of the list
					if (currKey == NULL) 
					{
						prev->next = newNote;
						newNote->next = currKey;
				...
				//Check for the case if the note is inserted
				//at the beginning
				if(listCheck == 0) {
					beat->notes = newNote;
					newNote->next = currKey;
