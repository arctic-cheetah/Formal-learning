...
int detect_square (int secondInput, int thirdInput, int fourthInput, int minefield[SIZE][SIZE]);
int reveal_square (int secondInput, int thirdInput, int minefield[SIZE][SIZE], int *gameWinp);
int print_gameplay_minefield (int gameOver, int minefield[SIZE][SIZE], int mineCountArray3x3[SIZE][SIZE]);
int reveal_radial (int secondInput, int thirdInput, int minefield[SIZE][SIZE], int *gameWinp, int mineCountArray3x3[SIZE][SIZE]);
int shift_mine_round1 (int minefield[SIZE][SIZE]);
...
int main(void) {
	...
	int mineCountArray3x3[SIZE][SIZE] = {0}; // This stores a map to count the number of mines on the minefield[][] array
    int mines = 0;
	...
	int gameMode = 6;
	int gameRound = 1;
	int hintCount = 0; //This counts the # of hints used
	int input1 = 0; //This is the first input

    while( gameOver != 1 && gameWin != 1 && scanf("%d", &input1) == 1){
     
    int input2 = 0; //Obtains a max of 4 inputs from the user
    int input3 = 0;       
    int input4 = 0;
    ...
    if(input1 == DEBUG_MODE){
        ...
        //Stage 1 commands: detect row and col
            ...
            //Stage 2 commands: detects # mines in a square
                ...
                detect_square (input2, input3, input4, minefield);
                hintCount++;
                if(gameMode == DEBUG_MODE){
                	print_debug_minefield (minefield);
				}
				...
				// This section shifts the grid down if the player steps on a mine on the first turn
					...
					if(minefield[input2][input3] == HIDDEN_MINE && gameRound == 1){
						shift_mine_round1 (minefield);				
						reveal_square (input2, input3, minefield, gameWinp);
						print_debug_minefield (minefield);
						mine_count(minefield, mineCountArray3x3);
					}
				...
				if(minefield[input2][input3] == HIDDEN_MINE && gameRound != 1){							
					...
					if(minefield[input2][input3] == HIDDEN_MINE && gameRound == 1){
						shift_mine_round1 (minefield);				
						reveal_radial (input2, input3, minefield, gameWinp, mineCountArray3x3);
						print_debug_minefield (minefield);
						mine_count (minefield, mineCountArray3x3);
					}
					else{
...
int detect_square (int secondInput, int thirdInput, int fourthInput, int minefield[SIZE][SIZE]) {
	...
	//This section prevents invalid input, such as row 10 col 59
...
int reveal_square (int secondInput, int thirdInput, int minefield[SIZE][SIZE], int *gameWinp) {
	...
	//This section prevents invalid input,
    if(secondInput > SIZE - 1){
        secondInput = SIZE - 1;
    }
    else if(secondInput < 0){
...
// loop through row, then column and count the # of mines
    int numberOfMines = 0;
    int i = rowLoopMin;
        while (i <= rowLoopMax ){
        int j = colLoopMin;
...
int print_gameplay_minefield (int gameOver, int minefield[SIZE][SIZE], int mineCountArray3x3[SIZE][SIZE]){
    ...
    printf("    ");
    for(int n = 0; n < SIZE; n++){  //Print top line with numbers
		if(n < 10){
			printf("0%d ", n);
		}
    	else{
    ...
    printf("   ");
    for(int m = 0; m <= 3*SIZE; m++){//Print top line
    	printf("-");
    }
    printf("\n");
	...
	printf("   ");
	for(int m = 0; m <= 3*SIZE; m++){ //Print bottom line
    	printf("-");
    }
    printf("\n");
...
int mine_count (int minefield[SIZE][SIZE], int mineCountArray3x3[SIZE][SIZE]) {
	...
	for(int corRow = 0; corRow < SIZE; corRow++){
		...
		//This section sets up boundaries for a 3x3 scan of the grid
		if (corRow == 0) {
			if (corCol == 0){
				rowLoopMin = 0;
				colLoopMin = 0;
	...
	// loop through row, then column---> This counts the # of mines in a 3x3 area
			if(minefield[corRow][corCol] == HIDDEN_MINE){
				...
				// The number nine (9) was chosen to represent a mine on the memory array
				//Because there are no commands associated with 9
			}		
			else{
			int numberOfMines = 0;
			int i = rowLoopMin;
