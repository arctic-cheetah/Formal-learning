 ...
 * operation was successful, and false if they were not friends.
 */
 ...
 *
 * Someone  should  be recommended if and only if they are a friend of a
 * friend of the person and they are not already a friend of the person.
 * In other words, someone should be recommended if  and  only  if  they
 * share at least one mutual friend with the person.
 *
 ...
 * of mutual friends shared with the given person, so people  with  more
 * mutual  friends  should be recommended first. If two people share the
 * same number of mutual friends, they may be printed in any order.
 *
 * For  each  recommendation,  print the person's name and the number of
 * mutual friends they share with the given person. The  format  of  the
