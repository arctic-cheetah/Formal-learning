	...
	while (searchWords[i] != NULL) {
		...
		while (nodesChecked != mergeLength) {
			...
			while (curr2->tfIdfSum != 0.0) {
				//insert in descending TfIdf order
				//But consider cases

				//Consider if inserting at the rear
				if (curr1 == NULL) {
				...
				if (curr2->tfIdfSum > curr1->tfIdfSum && curr1 == head) { 
					head = insertNode;
					insertNode->next = curr1;
					nodesChecked +=1;
					break;
				}
				else if (curr2->tfIdfSum > curr1->tfIdfSum) {
					prev1->next = insertNode;
					insertNode->next = curr1;
					nodesChecked +=1;
					break;
				}
				//If there are identical tfIdf values,
				//Highly unlikely due to inaccuracy with floating point
				//insert in lexical order
				else if (curr2->tfIdfSum == curr1->tfIdfSum) {
					while (strcmp(curr2->filename, curr1->filename) > 0) {
						prev1 = curr1;
						curr1 = curr1->next;
					}
					prev1->next = insertNode;
