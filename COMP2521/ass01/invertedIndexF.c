...
int specific_word_count (char *fileName, char *word) {
	...
	char currWord[MAX_LENGTH];
		//Need to count the number of terms in a document
		while (fscanf(fileRead, "%s", currWord) == 1) {
...
//WARNING!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
//Til is the acronym for: Tf Idf List
...
//Obtain a TfIdfList which will serve as the main list
	...
	while (searchWords[i] != NULL) {
		...
		while (nodesChecked != mergeLength) {
			curr1 = head;
			//Need to insert the nodes at the end
			//From the list that will be merged named "merge"
	...
	TfIdfList *array = malloc(sizeof(TfIdfList) * mainTil_count);
	TfIdfList curr = head;
	int k = 0;
	while (curr != NULL) {
		array[k] = curr;
	...
	//sort the tf values of the array
	//...Remember indexing begins from 0!
	sortArrayTil(array, mainTil_count - 1);

	TfIdfList sortedHead = copy_tilNode(array[0]);
	...
	while (j < mainTil_count) {
		...
		TfIdfList newNode = copy_tilNode(array[j]);
		curr->next = newNode;
		curr = curr->next;
		j +=1;
	}
	//Delete the old unsorted list and the array
	rm_tilNode(head);
	free(array);
	return sortedHead;
}
