...
void create_blob(char *blob_pathname, char *pathnames[], int compress_blob) {
    ...
    if (compress_blob) {
        //Get the name of the xz file
        char *xzFile = calloc(MAX_FILE_SIZE, sizeof(char));
        strcpy(xzFile, blob_pathname);
        xzFile = strcat(xzFile, ".xz");
        compressFile(blob_pathname);
        //Rename the compressed blob to the desired filename
        rename(blob_pathname);
        free(xzFile);
    }
}
//////////////////////////////////////////////////////////////////
