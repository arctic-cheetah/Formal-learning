...
void list_blob(char *blob_pathname) {
    ...
    if (c != BLOBETTE_MAGIC_NUMBER) {
    	fprintf(stderr, "First byte is not a magic number!\n");
    	exit(1);
    }
    ...
    while (i < blobSize) {
		...
		uint64_t mode = fetchByte(1, 3, f1);
		uint64_t pathname_length = fetchByte(4, 5, f1);
		uint64_t content_length = fetchByte(6, 11, f1);
		//Allocate memory for the length of the string
		char *pathName = malloc((pathname_length + 1) * sizeof(char));
		fetchFileName(12, 12 + pathname_length, f1, pathName);
		printf("%06lo %5lu %s\n", mode, content_length, pathName);
		
		//calculate the offset for the file
		i = mode + pathname_length + content_length;
    }
