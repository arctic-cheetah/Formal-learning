...
//MKXI: Removed debugging outputs, Ready for submission!
...
void compressFile(char *text, FILE *newBlob, 
                  uint64_t buffSize, char *blob_pathname);
...
void create_blob(char *blob_pathname, char *pathnames[], int compress_blob) {
    ...
    if (compress_blob) {
        ...
        compressFile(buffer, newBlob, size, blob_pathname);
    }
    fclose(newBlob);
}
//////////////////////////////////////////////////////////////////
...
void list_blob_not_compressed(char *blob_pathname) {
    ...
    while (i < blobSize - 1) {
		...
		uint64_t offset1 = (i + BLOBETTE_MODE_LENGTH_BYTES);
		...
		uint64_t offset3 = (offset2 + BLOBETTE_CONTENT_LENGTH_BYTES) + 1;
...
//A function that compresses a data
void compressFile(char *text, FILE *newBlob, uint64_t buffSize, 
                  char *blob_pathname) {
    ...
    //open the file
    int read = open(blob_pathname, (O_WRONLY | O_TRUNC));
    ...
    if (posix_spawn_file_actions_adddup2(&actions, read, 1) != 0) {
        perror("posix_spawn_file_actions_adddup2");
        exit(1);
    }
